...
웹에서 쓰던 이름과 비밀번호를 그대로 입력하면 됩니다. 별도 설정 과정은 없습니다.
비밀번호는 GitHub Actions Secrets(`VITE_ADMIN_PASSWORD`, `VITE_MEMBER_PASSWORD`)에서
빌드 시점에 주입되므로, 웹과 항상 같은 값이 됩니다.

> 비밀번호가 바뀌었는데 아직 새 버전을 안 받았다면,
> 메뉴 **파일 → 로그인 비밀번호 재설정…** (`Ctrl+,`)에서 이 PC에만 임시로 덮어쓸 수 있습니다.
> 저장 위치: `%APPDATA%\foundfoundedmeet\env.json`
...
로그인 창의 **[로그인 정보 저장]** 을 켜두면 다음에 앱을 켤 때 자동으로 로그인됩니다. (30일)
끄면 앱을 닫을 때 로그인 정보가 지워집니다.

> **왜 포트를 고정하는가**
> 앱은 내부적으로 `http://127.0.0.1:47321` 로 화면을 띄웁니다.
> localStorage 는 포트를 포함한 출처(origin) 단위로 저장되기 때문에,
> 포트를 매번 랜덤으로 잡으면 자동 로그인 토큰 · 가이드 확인 기록 · 테마 설정이
> 실행할 때마다 전부 초기화됩니다. 그래서 `electron/main.cjs` 에서 포트를 고정하고
> 한 번 잡은 포트를 `userData/port.json` 에 기록해 계속 재사용합니다.
> **이 부분을 랜덤 포트로 되돌리면 자동 로그인이 조용히 깨집니다.**
